...
<?php
/*
This script can be run at the command line via cron to search for all available jobs in US craigslist cities, input them into
a database and email you the results.

DB export is inside the folder for import.
...
//grab our cities 
$urls = get_urls();
//loop through our cities and add anything to new our db
foreach ($urls as $url)
	grab_feed($url);
//open our db
$database="clscrape";
$con = mysql_connect('***.***.***.***','****','****');
mysql_select_db('clscrape',$con) or die( 'Unable to select database' . mysql_error());
//get all jobs have remote in the description
$query =mysql_query("SELECT * FROM data WHERE date>=DATE_SUB(NOW(), INTERVAL 30 minute) AND data.desc LIKE '%remote%'");

//now lets grab all the links and titles, create an email body and email the results
$email_body = "New jobs in the past half hour\n\n";
while($row = mysql_fetch_array($query))
	$email_body = $email_body . $row['title'] . " - " . $row['link'] ."\n\n";
mysql_close($con);

require_once "Mail.php";

$from = "joe.a.armstrong@gmail.com";
$to = "joe.a.armstrong@gmail.com";
$subject = "New CL Jobs";
